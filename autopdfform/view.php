...
if (!empty($USER->username) && preg_match('/^(\d{5})@students\.hsrw/i', $USER->username, $m)) {
    // Extract the 5-digit number from username (before @)
    $student_id = $m[1];
} elseif (!empty($USER->idnumber) && preg_match('/^\d{5}$/', $USER->idnumber)) {
...
// Get the original template filename (without .pdf)
$templatefilename = $templatefile->get_filename();
$basename = pathinfo($templatefilename, PATHINFO_FILENAME);
// Truncate to max 60 characters if needed
if (mb_strlen($basename) > 60) {
    $basename = mb_substr($basename, 0, 60);
}
// Sanitize firstname/lastname for filenames (remove special chars, keep dash)
$firstname_safe = preg_replace('/[^A-Za-z0-9\-]/', '', $firstname);
$lastname_safe  = preg_replace('/[^A-Za-z0-9\-]/', '', $lastname);
// Compose final filename with dashes
$filename = $basename . '-' . $firstname_safe . '-' . $lastname_safe . '.pdf';
