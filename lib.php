...
function autopdfform_add_instance($data, $mform) {
    ...
    // Save uploaded template PDF into filearea 'template'.
    if (!empty($data->coursemodule) && !empty($data->templatefile)) {
...
function autopdfform_delete_instance($id) {
    ...
    // Find the associated course module to get the correct context.
    ...
    // Remove stored files if we have the context.
    if ($cm) {
...
function autopdfform_get_coursemodule_info($cm) {
    ...
    if (!$record) {
        return null; // Prevent course page crashes on partial data.
    }
    ...
    // Only show description on course page if the CM setting is enabled.
...
function autopdfform_view($autopdfform, $course, $cm, $context) {
    ...
    // Mark completion (harmless if completion isn't enabled for this CM).
